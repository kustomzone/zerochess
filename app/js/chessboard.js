...
function checkDeps() {
  ...
  if (! (typeof window.$ && $.fn && $.fn.jquery &&
    ...
    window.alert('ChessBoard Error 1005: Unable to find a valid version ' +
      'of jQuery. Please include jQuery ' + MINIMUM_JQUERY_VERSION + ' or ' + 'higher on the page.\n\nExiting...');
    return false;
  }
...
function expandConfig() {
  ...
  if (cfg.hasOwnProperty('pieceTheme') !== true || (typeof cfg.pieceTheme !== 'string' && typeof cfg.pieceTheme !== 'function')) {
    cfg.pieceTheme = 'app/img/chesspieces/{piece}.png';
  }
...
function buildPieceImgSrc(piece) {
  ...
  if (typeof cfg.pieceTheme === 'string') {
	...
	if (parts[0] == 'w') { pieceImg = pieceName + "_white" } else { pieceImg = pieceName + "_black" }
    return cfg.pieceTheme.replace(/{piece}/g, pieceImg);
  }
...
function buildPiece(piece, hidden, id) {
  ...
  if (hidden === true) { html += 'display:none;'; }
  html += '" />';
  return html;
}
...
function buildSparePieces(color) {
  ...
  var piecez = ['king_white', 'queen_white', 'rook_white', 'bishop_white', 'knight_white', 'pawn_white'];
  if (color === 'black') {
    piecez = ['king_black', 'queen_black', 'rook_black', 'bishop_black', 'knight_black', 'pawn_black'];
  }
  var html = '';
  for (var i = 0; i < pieces.length; i++) {
